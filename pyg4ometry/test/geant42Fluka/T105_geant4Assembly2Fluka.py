...
def Test(vis = False, interactive = False, fluka = True) :
    ...
    if fluka :
        ...
        w = _fluka.Writer()
        w.addDetector(freg)
        w.write(_os.path.join(_os.path.dirname(__file__),"T105_geant4Assembly2Fluka.inp"))

    # flair output file
    f = _fluka.Flair("T105_geant4Assembly2Fluka.inp",extentBB)
    f.write(_os.path.join(_os.path.dirname(__file__),"T105_geant4Assembly2Fluka.flair"))
