...
def Test(vis = False, interactive = False) :
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(_os.path.join(_os.path.dirname(__file__), "T023_geant4TwistedTrd2Fluka.gdml"))

    # fluka conversion
    freg = _convert.geant4Logical2Fluka(wl)
    w = _fluka.Writer()
    w.addDetector(freg)
    w.write(_os.path.join(_os.path.dirname(__file__),"T023_geantTwistedTrd2Fluka.inp"))

    # flair output file
    f = _fluka.Flair("T024_geant4TwistedTrd2Fluka.inp",extentBB)
    f.write(_os.path.join(_os.path.dirname(__file__),"T024_geant4TwistedTrd2Fluka.flair"))
