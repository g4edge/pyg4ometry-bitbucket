...
def MakeECamelAssembly(reg, margin=1, separation=1):
    ...
    polygon = [[-dx,0],[-dx,dy],[0,dy],[0,dy-tt],[-dx+tt,dy-tt],
               [-dx+tt,0.5*(dy+tt)],[0,0.5*(dy+tt)],[0,0.5*dy-0.5*tt],
               [-dx+tt,0.5*dy-0.5*tt],[-dx+tt,tt],[0,tt],[0,0]]
    ...
    ma = margin
    hy = 0.5*(dy - 3*tt)
    polygon2 = [[0,0],[0,tt+ma],[-hx,tt+ma],[-hx,tt+hy-ma],
               [0,tt+hy-ma],[0,tt+hy+tt+ma],[-hx, tt+hy+tt+ma],[-hx,dy-tt-ma],
               [0,dy-tt-ma],[0,dy],[tt,dy],[tt,0]]
    ...
    # structure
    assembly = _g4.AssemblyVolume("part", reg)
    xo = (dx + tt)*0.5 - tt
