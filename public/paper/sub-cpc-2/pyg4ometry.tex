...
available and geometry conversion is difficult or impossible between these simulation tools.
\\
...
geometry and to enable the use of advanced mesh-based geometric algorithms. Triangular mesh processing algorithms allow the loading and use of Standard Triangle Language (STL) and CAD/CAM
...
There are numerous software codes to simulate the passage of particles through material. Such radiation transport (RT) programs
...
All these codes are based on the Monte Carlo technique but each code either has a particular speciality, simulation methodology or target user community.
...
computer-readable description of the  physical 3D geometry that  the particles are passing through.  The creation of geometry description files is
...
and IGES) based on FreeCAD~\cite{FreeCAD} and OpenCascade~\cite{OpenCASCADE}; and  geometry visualisation tools based on VTK~\cite{VTK4}. The
...
can load, display and manipulate GDML-like geometry. ROOT is typically harder for the layperson (or non-high energy physicist) to use compared to the python implementation of \PYGEOMETRY{}. Existing open source tools to convert CAD files to GDML include GUIMesh~\cite{GUIMesh_Pinto}, with commercial
...
(or other similar mesh formats), which can be used by Geant4~\cite{poole2012acad}. However, in practice using CAD models is difficult if that model is composed of a
...
or FLUKA and the existing set of software does not provide a complete set of tools to efficiently create complex geometries for these codes.

This paper is structured as follows: first comes a brief radiation transport-focused introduction to computer descriptions of geometry and an explanation of the design
...
Central to a computer-readable geometry is how a solid is defined in three dimensions. There are numerous ways to describe a
...
performing an intersection operation between solid mesh instances and determining if the resulting mesh is empty. Figure~\ref{fig:overlap} shows three different types of possible
...
descriptions which have a high degree of logical volume reuse, which this is also true of Geant4 as a whole.
Due to the discrete nature of triangular meshes it is not possible to have perfect detection
of overlaps, especially when curved surfaces are considered. In some rare
cases either overlaps maybe missed, or spurious overlaps maybe reported.  However, the density of the meshes
created for the solids is controllable by the user, so the user can opt for more precise overlap checking at the cost of greater computation time.  
In general, the overlap detection algorithm can present the potential overlaps quickly and easily to the user, so significantly aid the geometry 
debugging process.
...
# physical volume visualization attributes
...
\pyinline{EllipticalTube} and \pyinline{Box} instances (user adjustable but default
...
straightforward as the two formats are similarly expressive.
...
computationally tractable, and in doing so FLUKA's tracking will be faster.
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\columnwidth]{SectorBend}
...
An example geometry representing a dipole electromagnet consisting of three parts was created in a commercial CAD  package (Autodesk Fusion 360)
...
from BDSIM), a sector bend dipole electromagnet (created in a commercial CAD package) and finally a Faraday cup (FLUKA geometry designed in
...
implemented in the current version but could be relatively quickly implemented for users who
...
optical photon based particle physics detectors. VTK/Paraview is becoming standard software for 
...
{\bf Stewart Boogert}: conceptualization, supervision, methodology, software, validation and writing - original draft. 
{\bf Stuart Walker}: methodology, software, validation and writing - original draft (FLUKA components). 
{\bf Andrey Abramov}: software, validation and writing - review \& editing (Geant4 and GDML). 
{\bf Laurence Nevay}: software, validation and writing - review \& editing. 
{\bf William Shields}: software (GDML), validation and writing - review \& editing
