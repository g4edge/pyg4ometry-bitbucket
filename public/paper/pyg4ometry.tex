...
\textit{zone} consists of one or more bodies combined using
intersections and subtractions.  Zones may then be further combined
using union operations to form \textit{regions}, which is defined as
the combination of one or more zones as well as a material.  \todo{nested zones.}

Translating FLUKA geometry to GDML first starts with 

Translating FLUKA geometry to GDML first starts with a mapping of
FLUKA bodies to GDML solids, which is shown in
Table~\ref{tab:Fluka2Geant4}.  

It is notable that FLUKA makes
widespread use of primitives that are infinite in extent, where Geant4 

FLUKA geometry makes widespread use of primitives that are infinite in
extent, with this feature distinguishing it most from Geant4 where all
primitives are finite.

FLUKA geometry is most distinuished from Geant4 by its use of
primitives infinite in extent.

Bodies can be combined into
\textit{zones} using intersections and subtractions, with 

Bodies may be
combined into \textit{zones} intersections and subtractions, and these
zones may then be combined into \textit{regions} with union
operations.  The most important difference between Geant4 and FLUKA's
implementation of CSG is that FLUKA makes widespread use of primitives
that are infinite in extent, whereas all Geant4 primitives are finite.
Whereas some FLUKA primitives have trivial Geant4 analogues, for
example a sphere, translating the infinite bodies to Geant4 solids is
more involved.  The mapping between FLUKA bodies and Geant4 solids is
shown in Table~\ref{tab:Fluka2Geant4}.

Many FLUKA primitives are infinite in extent, so to emulate
FLUKA geometry construction large but finite extent Geant4 primitive
solids are used.  shows the mapping
between FLUKA and Geant4 objects.  An ANTLR based expression parser is
used to load and comprehend the FLUKA input syntax, an example is
shown in XXX. Each FLUKA body is matched to a Geant4 solid and a FLUKA
region is represented by a Geant4 CSG tree is formed using the
pyg4ometry classes.
