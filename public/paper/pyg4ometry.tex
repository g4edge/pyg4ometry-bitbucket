...
particle transport code users to rapidly develop error free geometry files, convert between common RT programme geometry formats and finally import CAD models. 
\colorbox{yellow}{More explanation of Geant4 and FLUKA} 

This paper describes a geometry creation and conversion package called PYG4OMETRY, implemented in PYTHON and internally based on the Geant4 for APIs and Geometry Description Markup Language (GDML) for file persistency  \cite{GDML}. The main features of PYG4OMETRY are a PYTHON scripting API to rapidly design parametrised geometry, conversion to and from  FLUKA geometry descriptions, conversion from CAD formats  (STEP and IGES) based on FreeCAD \cite{FreeCAD} and OpenCascade \cite{OpenCASCADE} and powerful geometry visualisation tools based on VTK \cite{VTK4}. The origin of PYG4OMETRY was a set of utilities to prepare geometry for an accelerator beamline simulation based on Geant4 called BDSIM \cite{BDSIM_Nevay}. Accelerator physicists, like specialists in other areas, need a tool to quickly simulate beam losses and the subsequent interaction of the charged particle beam with the accelerator material. Figure~\ref{fig:workflow} depicts various workflows possible with PYG4OMETRY. PYG4OMETRY is not an executable software package but a toolkit, a user would typically write very small PYTHON program to use the PYG4OMETRY. This paper describes version 1.0 of PYG4OMETRY, which is freely available as a git repository and via PyPi. 

\begin{figure*}[hbt!]
  \normalsize
  \centering
  \includegraphics[width=0.7\textwidth]{./diagrams/workflow.pdf}
  \caption{\label{fig:workflow}Schematic of PYG4OMETRY workflow, showing a) the different input file formats, b) PYTHON processing, c) output file targets and d) RTMC codes which use the geometry. }
\end{figure*}
...
This paper is structured as follows, first a brief radiation transport-centric introduction to computer descriptions of geometry, followed by an explanation of the design and implementation of PYG4OMETRY. The sections that follow describe how PYG4OMETRY can be used to perform rapid geometric modelling, conversion from FLUKA to GDML, GDML to FLUKA and CAD to GDML. The paper concludes with an example of composite complex system consisting components of all the possible geometry input files formats.
...
To exchange geometry descriptions between software packages Geometry Descripition Mark-up Language (GDML) was developed \cite{GDML}. 
...
PYG4OMETRY is a PYTHON package consisting of semi-independent sub-packages. The sub-package \verb|pyg4ometry.geant4| contains all classes for 
geant4 detector construction, \verb|pyg4ometry.gdml| provides the functionality for reading and writing GDML files. There are sub-packages for importing and exporting other geometry formats  \verb|pyg4ometry.fluka|, \verb|pyg4ometry.stl| and \verb|pyg4ometery.freecad| and converting between file formats \verb|pyg4ometry.convert|.

The core of PYG4OMETRY consists of PYTHON classes that mimic Geant4 solids, logical, physical volume objects, GDML parameters and material classes.
...
\begin{lstlisting}[caption={A simple PYTHON script using PYG4OMETRY to create GDML variables},label={lst:gdmlExpressions}, language=Python]
# Import modules 
import pyg4ometry
...
instances of geometry (physical volumes) and meshes for CSG derived solids. CSG mesh calculations are performed using a BSP tree technique in pure PYTHON 
\cite{pycsg} or the Computational Geometry Algorithms Library (CGAL) surface meshes \cite{cgal:bsmf-sm-20b}. In general the CGAL implementation is one or two orders of magnitude faster than the pure-PYTHON CSG implementation and must be used for large geometries. Triangular meshes based on CSG 
...
The {\it Physical Volume} class (\verb|pyg4ometry.geant4.PhysicalVolume|) is also used to store visualisation attributes like the solid's 
...
through such a situation.  A key feature of PYG4OMETRY is the detection of potential overlaps efficiently, it does this by performing an intersection operation between solid instances and determining if the resulting mesh is empty. Figure~\ref{fig:overlap} shows the potential overlaps, a) protrusion of a daughter from the mother, b)  finite volume intersection between two daughters  and c) an overlap where two daughters share a face. 
\begin{figure}[htbp]
\begin{center}
...
users of Geant4 or GDML, apart from a new class called the \verb|Registry|. First a \verb|Registry| need to be created to store all the PYG4OMETRY objects; followed by constants;  then materials and properties, solids and logical and physical volumes; finally the whole geomtery can be saved as a GDML file and/or visualised using VTK.
...
programatic converters between different geometry languages or more generally 
...
is rather trivial, a significantly more complex example is in Figure~\ref{fig:gdml-flair}.
...
has XXX bodies and in general infinite extent bodies have benefits over finite ones. Key 
for conversion are XY-, XZ-, YZ-Panes (XYP, XZP, YZP), arbitrary plane (PLA), 
X, Y,  Z axis aligned cylinders (XCC, YCC, ZCC), X, Y, Z axis aligned elliptical cylinders (XEC, YEC, ZEC), 
...
\colorbox{yellow}{XXX}. PYG4OMETRY can simplify parentheses from a \verb|REGION| by creating a 
