...
of topology present in traditional CAD BREP systems. One of the fundamental solids is a tessellated sollid which can used to represent STL or PLY files. 
...
and zero or more physical {\em daughter} physical volumes. A physical volume is a placement (or instance) of a logical volume.   
...
%%%%%%%%%%%%%%%%% Volume bounding
If a user is creating a placing multiple daughter volumes within a {\em mother} volume then it is the users responsibility to create a solid which fully 
...
to a logical volume. \cpinline{G4Material} has two main constructors, the first were a atomic number is supplied and 
...
card. Geant4 also defines a set of standard materials \cite{NIST} from NIST so a user can just specify something like 
\pyinline{G4_STAINLESS-STEEL}, \PYGEOMETRY contains a matching database and creates the appropriate FLUKA cards during 
the conversion.
...
restricting daughter volumes in Geant4 that can be expressed without 
XXX. If \PYGEOMETRY{} is used during the creation of geometry then multiple 
codes can be targeted without additional user effort. There are still a few outstanding technical 
issues, which should be discussed. 

Currently replica, division and parametrised placements are not implemented 
and will be addedin a future release. In Geant4 it is possible to create scaled 
...
layer of the twisted solid as a union of terahedra. A similar problem 
...
\begin{enumerate}
\item Coherent, uniform interface to existing tools
...
\item Performance in Geant4/FLUKA
\item AR/VR export (to OBJ or XXX)
\item Where to put manual, unit tests, installation, git? 
\item range of current users (RAL, CERN, LUXE, QUB, IHEP-CEPC, etc) 
\item Paraview loader 
\item 2D section viewer 
\item PHITS, MCNP(X)
\end{enumerate}
