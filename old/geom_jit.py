...
class Plane(object):
    ...
    @staticmethod
    @jit(nopython=True)
    def _jit_splitPolygon(vpos, normal, w, EPSILON=1.e-5):
        ...
        for i in range(len(vpos)):
            t = vpos[i][0]*normal[0] + vpos[i][1]*normal[1] + vpos[i][2]*normal[2] - w
            loc = -1
            if t < -EPSILON:
                loc = BACK
            elif t > EPSILON:
    ...
    def splitPolygon(self, polygon, coplanarFront, coplanarBack, front, back):
        ...
        vpos = _np.array([[v.pos.x, v.pos.y, v.pos.z] for v in polygon.vertices])
        pnormal = [self.normal.x, self.normal.y, self.normal.z]
        polygonType, vertexLocs = self._jit_splitPolygon(vpos, pnormal, self.w)
